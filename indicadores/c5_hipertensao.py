...
def render():
    st.header("🫀 Auditoria Indicador C5 — Cuidado da Pessoa com Hipertensão")
    uploaded_file = st.file_uploader("Anexe o relatório CSV de Hipertensão:", type=["csv"], key="uploader_c5")
    
    if uploaded_file and st.button("🚀 Processar Dashboard C5", key="btn_c5"):
        with st.spinner("Processando C5..."):
            df, data_ref = carregar_e_limpar_csv(uploaded_file.getvalue())
            df_micro, df_nom = processar_c5(df, data_ref)
            cols_status = [c for c in df_nom.columns if "Prática" in c or "status" in c]
            excel_bytes = gerar_excel_padrao(df_micro, df_nom, "INDICADOR C5 — CUIDADO DA PESSOA COM HIPERTENSÃO", data_ref, cols_status, "Score_C5_%")
            
            st.success("✅ Dashboard C5 gerado!")
            st.download_button("📥 Baixar Dashboard_Indicador_C5.xlsx", excel_bytes, "Dashboard_Indicador_C5.xlsx")
