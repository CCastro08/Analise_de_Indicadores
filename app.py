...
"""Busca insensível a maiúsculas/minúsculas por nome de coluna contendo palavras-chave."""
for col in df.columns:
col_norm = col.lower()
if all(kw.lower() in col_norm for kw in palavras_chave):
return col
return None

def obter_valor_coluna(row, df, palavras_chave, padrao=None):
col = buscar_coluna(df, palavras_chave)
if col and col in row:
val = row[col]
return val if val is not None else padrao
return padrao

--- GERAÇÃO DE EXCEL PADRONIZADO ---
...
col_nasc = buscar_coluna(df, ['nascimento']) or 'Data de nascimento'
df['Nascimento'] = pd.to_datetime(df.get(col_nasc), format='%d/%m/%Y', errors='coerce')
df['Idade_Dias'] = (data_ref - df['Nascimento']).dt.days
df = df[df['Idade_Dias'] < 1096].copy()

def eval_c2(row):
    dias = row.get('Idade_Dias', 0)
    p_a = "Não atendida"
    d_cons = obter_valor_coluna(row, df, ['primeira', 'consulta'])
    if d_cons:
        try:
    ...
    qtd_cons = int(pd.to_numeric(obter_valor_coluna(row, df, ['consultas', 'puericultura'], 0), errors='coerce') or 0)
    p_b = "Atendida" if qtd_cons >= 9 else ("Aguardando idade" if dias < 730 else "Não atendida")

    qtd_ant = int(pd.to_numeric(obter_valor_coluna(row, df, ['medições', 'simultâneas'], 0), errors='coerce') or 0)
    p_c = "Atendida" if qtd_ant >= 9 else ("Aguardando idade" if dias < 730 else "Não atendida")

    dts_v = extrair_datas_validas(obter_valor_coluna(row, df, ['visitas']), data_ref, 180)
    p_d = "Atendida" if len(dts_v) >= 2 else ("Aguardando idade" if dias < 180 else "Não atendida")

    sit_vac = str(obter_valor_coluna(row, df, ['vacinal'], '')).lower()
    p_e = "Atendida" if 'em dia' in sit_vac or 'sim' in sit_vac else ("Aguardando idade" if dias < 365 else "Não atendida")
...
res_c2 = df.apply(eval_c2, axis=1)
for col in res_c2.columns: df[col] = res_c2[col]
...
"""Função resiliente para cálculo do Indicador C3 (Gestantes e Puérperas)."""
if df.empty:
...
return pd.DataFrame(columns=['Microárea', 'Gestantes_Puerperas_Ativas', 'Score_Médio_C3', 'Pct_100_Avaliável']), pd.DataFrame(columns=cols_nom)

col_dum = buscar_coluna(df, ['dum'])
col_ig = buscar_coluna(df, ['idade', 'gestacional'])

def calcular_ig_dias(row):
    # Tenta calcular via DUM primeiro
    if col_dum and row.get(col_dum):
        val_dum = str(row[col_dum]).strip()
        for fmt in ['%d/%m/%Y', '%Y-%m-%d', '%d-%m-%Y']:
            try:
                dt_dum = datetime.strptime(val_dum[:10], fmt)
                dias = (data_ref - dt_dum).days
                if 0 <= dias <= 350:
                    return dias
            except: pass
    
    # Tenta extrair IG da coluna de texto se a DUM falhar
    if col_ig and row.get(col_ig):
        txt_ig = str(row[col_ig]).lower()
        match_sem = re.search(r'(\d+)\s*(?:sem|semana)', txt_ig)
        match_dias = re.search(r'(\d+)\s*(?:dia)', txt_ig)
        dias_totais = 0
        if match_sem:
            dias_totais += int(match_sem.group(1)) * 7
        if match_dias:
            dias_totais += int(match_dias.group(1))
        if dias_totais > 0:
            return dias_totais

    # Se não houver DUM nem IG, assume gestação ativa (140 dias = 20 semanas) para evitar descarte indevido
    return 140

df['IG_Dias_Calc'] = df.apply(calcular_ig_dias, axis=1)

# Manter gestantes (0-294d) e puérperas (295-336d)
df = df[(df['IG_Dias_Calc'] >= 0) & (df['IG_Dias_Calc'] <= 336)].copy()
...
col_att_12sem = buscar_coluna(df, ['12', 'semanas']) or buscar_coluna(df, ['atendimentos', 'pré-natal'])
col_cons_pn = buscar_coluna(df, ['consultas', 'pré-natal'])
col_pa = buscar_coluna(df, ['pressão'])
col_ant = buscar_coluna(df, ['peso', 'altura'])
col_vis = buscar_coluna(df, ['visitas'])
col_dtpa = buscar_coluna(df, ['dtpa'])
col_puerp_cons = buscar_coluna(df, ['puerpério', 'consulta']) or buscar_coluna(df, ['consulta', 'puerpério'])
col_puerp_vis = buscar_coluna(df, ['puerpério', 'visita']) or buscar_coluna(df, ['visita', 'puerpério'])
col_odonto = buscar_coluna(df, ['odontológico']) or buscar_coluna(df, ['bucal'])

def eval_c3(row):
    ig = row.get('IG_Dias_Calc', 140)
    is_puerpera = ig > 294
    
    v_a = int(pd.to_numeric(row.get(col_att_12sem, 0), errors='coerce') or 0) if col_att_12sem else 0
    p_a = "Atendida" if v_a >= 1 else "Não atendida"
    
    v_b = int(pd.to_numeric(row.get(col_cons_pn, 0), errors='coerce') or 0) if col_cons_pn else 0
    p_b = "Atendida" if v_b >= 7 else "Não atendida"
    
    v_c = int(pd.to_numeric(row.get(col_pa, 0), errors='coerce') or 0) if col_pa else 0
    p_c = "Atendida" if v_c >= 7 else "Não atendida"
    
    v_d = int(pd.to_numeric(row.get(col_ant, 0), errors='coerce') or 0) if col_ant else 0
    p_d = "Atendida" if v_d >= 7 else "Não atendida"
    
    v_e = int(pd.to_numeric(row.get(col_vis, 0), errors='coerce') or 0) if col_vis else 0
    p_e = "Atendida" if v_e >= 3 else "Não atendida"
    
    dtpa_val = str(row.get(col_dtpa, '') or '').strip() if col_dtpa else ''
    p_f = "Atendida" if dtpa_val and dtpa_val.lower() not in ['-', 'none', 'nan', ''] else ("Aguardando idade" if ig < 140 else "Não atendida")
    
    ex_1t = [row.get(c) for c in df.columns if '1ºt' in c.lower() or '1t' in c.lower()]
    p_g = "Atendida" if ex_1t and all([str(x or '').upper() in ['SIM', 'REALIZADO', '1'] for x in ex_1t]) else ("Aguardando idade" if ig <= 97 else "Não atendida")

    ex_3t = [row.get(c) for c in df.columns if '3ºt' in c.lower() or '3t' in c.lower()]
    p_h = "Atendida" if ex_3t and all([str(x or '').upper() in ['SIM', 'REALIZADO', '1'] for x in ex_3t]) else ("Aguardando idade" if ig < 196 else "Não atendida")

    c_puerp = row.get(col_puerp_cons) if col_puerp_cons else None
    p_i = ("Atendida (1/1)" if c_puerp and str(c_puerp).lower() not in ['-', 'none', ''] else "Não atendida (0/1)") if is_puerpera else "Não se aplica"
    
    v_puerp = row.get(col_puerp_vis) if col_puerp_vis else None
    p_j = ("Atendida (1/1)" if v_puerp and str(v_puerp).lower() not in ['-', 'none', ''] else "Não atendida (0/1)") if is_puerpera else "Não se aplica"
    
    v_odo = int(pd.to_numeric(row.get(col_odonto, 0), errors='coerce') or 0) if col_odonto else 0
    p_k = "Atendida (1/1)" if v_odo >= 1 else "Não atendida"
...
def eval_c4(row):
    def check_janela(kw, dias):
        val = obter_valor_coluna(row, df, kw)
        if val:
            try:
    ...
    p_a = check_janela(['última', 'consulta'], 183)
    p_b = check_janela(['pressão', 'arterial'], 183)
    p_d = check_janela(['peso', 'altura'], 365)
    
    d_eval = obter_valor_coluna(row, df, ['avaliação', 'glicada'])
    d_sol = obter_valor_coluna(row, df, ['solicitação', 'glicada'])
    ...
    p_f = check_janela(['avaliação', 'pés'], 365)
    dts_v = extrair_datas_validas(obter_valor_coluna(row, df, ['visitas']), data_ref, 365)
...
res_c4 = df.apply(eval_c4, axis=1)
for col in res_c4.columns: df[col] = res_c4[col]
...
res_c5 = df.apply(eval_c5, axis=1)
for col in res_c5.columns: df[col] = res_c5[col]
...
def eval_c6(row):
    d_med = pd.to_numeric(obter_valor_coluna(row, df, ['médico']), errors='coerce')
    d_enf = pd.to_numeric(obter_valor_coluna(row, df, ['enfermagem']), errors='coerce')
    p_a = "Atendida (1/1)" if ((pd.notnull(d_med) and 0 <= d_med <= 365) or (pd.notnull(d_enf) and 0 <= d_enf <= 365)) else "Não atendida (0/1)"

    qtd_ant = int(pd.to_numeric(obter_valor_coluna(row, df, ['simultâneos'], 0), errors='coerce') or 0)
    ...
    flu = str(obter_valor_coluna(row, df, ['influenza'], '') or '').strip()
    p_d = "Atendida (1/1)" if flu and flu.lower() not in ['-', 'sem registro', 'none', ''] else "Não atendida (0/1)"
...
res_c6 = df.apply(eval_c6, axis=1)
for col in res_c6.columns: df[col] = res_c6[col]
...
def eval_c7(row):
    ...
    if 25 <= idade <= 64:
        d_sol = obter_valor_coluna(row, df, ['colo', 'solicitação'])
        d_eval = obter_valor_coluna(row, df, ['colo', 'avaliação'])
    ...
    if 9 <= idade <= 14:
        hpv = str(obter_valor_coluna(row, df, ['hpv'], '') or '').strip()
        p_b = "Atendida (1/1)" if hpv and hpv.lower() not in ['-', 'sem registro', 'none', ''] else "Não atendida (0/1)"

    # Prática C (Saúde Sexual/Reprodutiva 14-69a, 12m)
    p_c = "Fora da faixa etária"
    if 14 <= idade <= 69:
        d_ssr = obter_valor_coluna(row, df, ['sexual', 'reprodutiva'])
    ...
    if 50 <= idade <= 69:
        d_sol = obter_valor_coluna(row, df, ['mama', 'solicitação'])
        d_real = obter_valor_coluna(row, df, ['mama', 'realização'])
        d_eval = obter_valor_coluna(row, df, ['mama', 'avaliação'])
...
res_c7 = df.apply(eval_c7, axis=1)
for col in res_c7.columns: df[col] = res_c7[col]
